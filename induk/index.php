...
$data = query("SELECT * FROM master");
// total titik pju
$t_titik = count($data);

// total watt
$t_watt = query("SELECT SUM(daya_sedir_watt) FROM master");
$t_watt = $t_watt[0]["SUM(daya_sedir_watt)"];

// total va
$t_va = query("SELECT SUM(daya_sedir_va) FROM master");
$t_va = $t_va[0]["SUM(daya_sedir_va)"];

// total meterisasi
$t_meterisasi = COUNT(query("SELECT * FROM master WHERE klasifikasi = 'meterisasi'"));
// total tersebar
$t_tersebar = COUNT(query("SELECT * FROM master WHERE klasifikasi = 'tersebar'"));
// total swadaya
$t_swadaya = COUNT(query("SELECT * FROM master WHERE klasifikasi = 'swadaya'"));

// watt meterisasi
$w_meterisasi = query("SELECT SUM(daya_sedir_watt) FROM master WHERE klasifikasi = 'meterisasi'");
$w_meterisasi = $w_meterisasi[0]["SUM(daya_sedir_watt)"];
// watt tersebar
$w_tersebar = query("SELECT SUM(daya_sedir_watt) FROM master WHERE klasifikasi = 'tersebar'");
$w_tersebar = $w_tersebar[0]["SUM(daya_sedir_watt)"];
// watt swadaya
$w_swadaya = query("SELECT SUM(daya_sedir_watt) FROM master WHERE klasifikasi = 'swadaya'");
$w_swadaya = $w_swadaya[0]["SUM(daya_sedir_watt)"];

// va meterisasi
$v_meterisasi = query("SELECT SUM(daya_sedir_va) FROM master WHERE klasifikasi = 'meterisasi'");
$v_meterisasi = $v_meterisasi[0]["SUM(daya_sedir_va)"];
// va tersebar
$v_tersebar = query("SELECT SUM(daya_sedir_va) FROM master WHERE klasifikasi = 'tersebar'");
$v_tersebar = $v_tersebar[0]["SUM(daya_sedir_va)"];
// va swadaya
$v_swadaya = query("SELECT SUM(daya_sedir_va) FROM master WHERE klasifikasi = 'swadaya'");
$v_swadaya = $v_swadaya[0]["SUM(daya_sedir_va)"];

// total normal
$t_normal = COUNT(query("SELECT * FROM master WHERE kondisi = 'normal'"));
// total nyala
$t_nyala = COUNT(query("SELECT * FROM master WHERE kondisi LIKE '%nyala%'"));
// total rusak
$t_rusak = COUNT(query("SELECT * FROM master WHERE kondisi = 'rusak'"));

// total request
$t_request = COUNT(query("SELECT * FROM request"));
            ...
            <div class="row r4">
                <div class="col-table">
                    <table>
                        ...
                        <?php
                            // Hitung jumlah kota
                            $kab_kota_all = query("SELECT DISTINCT kab_kota FROM master");
                            $j_kab_kota = intval(query("SELECT COUNT(DISTINCT kab_kota) FROM master")[0]["COUNT(DISTINCT kab_kota)"]);
                        ?>

                        <?php
                            // Hitung jumlah ulp
                            $temp = 0;
                            $temp_ulp = query("SELECT DISTINCT ulp FROM master");
                            $j_temp_ulp = intval(query("SELECT COUNT(DISTINCT ulp) FROM master")[0]["COUNT(DISTINCT ulp)"]);
                        ...
                        <?php for($j=0; $j<$j_kab_kota; $j++): ?>
                            ...
                            <?php
                                // Hitung jumlah ulp
                                $ulp_all = query("SELECT DISTINCT ulp FROM master WHERE kab_kota LIKE '$kab_kota'");
                                $j_ulp = intval(query("SELECT COUNT(DISTINCT ulp) FROM master WHERE kab_kota LIKE '$kab_kota'")[0]["COUNT(DISTINCT ulp)"]);
                            ?>
