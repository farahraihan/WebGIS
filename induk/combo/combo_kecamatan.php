<?php
    ...
    if($modul=='Desa'){
        $query = query("SELECT DISTINCT desa FROM master WHERE kecamatan LIKE '%$id%'");
    ...
    if($modul=='Klasifikasi'){
        $query = query("SELECT DISTINCT klasifikasi FROM master WHERE kecamatan LIKE '%$id%'");
        $j_query = intval(count($query));
    ...
    if($modul=='Idpel'){
        $query = query("SELECT DISTINCT id_pel FROM master WHERE kecamatan LIKE '%$id%'");
        $j_query = intval(count($query));
