<?php
    ...
    if($modul=='Klasifikasi'){
        $query = query("SELECT DISTINCT klasifikasi FROM master WHERE desa LIKE '%$id%' AND ulp LIKE '%$ulp%'");
    ...
    if($modul=='Idpel'){
        $query = query("SELECT DISTINCT id_pel FROM master WHERE desa LIKE '%$id%' AND ulp LIKE '%$ulp%'");
        $j_query = intval(count($query));
