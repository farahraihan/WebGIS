<?php
    ...
    if($modul=='Kecamatan'){
        $query = query("SELECT DISTINCT kecamatan FROM master WHERE kab_kota LIKE '%$id%' AND ulp LIKE '%$ulp%'");
    ...
    if($modul=='Desa'){
        $query = query("SELECT DISTINCT desa FROM master WHERE kab_kota LIKE '%$id%' AND ulp LIKE '%$ulp%'");
        $j_query = intval(count($query));
    ...
    if($modul=='Klasifikasi'){
        $query = query("SELECT DISTINCT klasifikasi FROM master WHERE kab_kota LIKE '%$id%' AND ulp LIKE '%$ulp%'");
        $j_query = intval(count($query));
    ...
    if($modul=='Idpel'){
        $query = query("SELECT DISTINCT id_pel FROM master WHERE kab_kota LIKE '%$id%' AND ulp LIKE '%$ulp%'");
        $j_query = intval(count($query));
